...
function [] = image_stitching(input_A, input_B)
% -------------------------------------------------------------------------
% 1. 加载两个图像，转换为双倍和灰度。
% 2. 检测两幅图像的特征点。
% 3. 在两幅图像的每个关键点周围提取固定大小的patches，并通过简单地将每个patch中的像素值“平坦化”到一维向量来形成描述符。
% 4. 计算一张图像中的每个描述符与另一张图像中的每个描述符之间的距离。
% 5. 根据上述得到的两两描述符距离矩阵选择假定匹配。
% 6. 运行RANSAC来估计(1)仿射变换和(2)将一张图像映射到另一张图像上的单应性。
% 7. 使用估计的变换将一个图像扭曲到另一个图像上。
% 8. 创建一个足够大的新图像来容纳全景图，并将这两张图像合成到其中。
%
% Input:
% input_A - filename of warped image（弯曲图像）
% input_B - filename of unwarped image
% Output:
% output_image - combined new image

% READ IMAGE, GET SIZE INFORMATION
% image_A = imread('C:\Users\Test\Desktop\Retina OCTA_2.bmp');
% image_B = imread('C:\Users\Test\Desktop\Retina OCTA_1.bmp');
image_A =input_A;
image_B = input_B;
...
sigma = 7;
[des_A] = getFeatureDescriptor(gray_A, x_A, y_A);
[des_B] = getFeatureDescriptor(gray_B, x_B, y_B);
...
% 使用4点RANSACto计算鲁棒单应性估计，保持第一张图像不扭曲。从第二到第一个
...
% ------------------------------- other functions -------------------------
% -------------------------------------------------------------------------
% function [xp, yp, value,t] = harris(input_image, sigma,thd, r)
...
% Extract non-rotation invariant feature descriptors（提取非旋转不变的特征描述符）
% 输入:输入灰度值图像
% xx - x 潜在特征点坐标
% yy - y 潜在特征点坐标
% output:
